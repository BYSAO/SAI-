package com.system.service;

import com.system.common.entity.FileSystem;
import com.system.service.FileSystemService;


import com.system.common.entity.Business;
import com.system.provider.restTemplate.BusinessPService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.util.StringUtils;

import java.util.ArrayList;
import java.util.List;

@Service
public class BusinessService {
    @Autowired
    private BusinessPService BusinessDao;
    @Autowired
    private FileSystemService FileSystemService;


    //查询所有内容
    public List<Business> getAll(Business t) {
        List<Business> allT = new ArrayList<>();
        try {
            List<Business> allSearch = BusinessDao.getAll(t);
            for (Business all : allSearch) {
                Business search = new Business();
                search.setId(all.getId());
                allT.add(get(search));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return allT;
    }

    //分页查询内容
    public List<Business> getAll2Page(Business t) {
        List<Business> allT = new ArrayList<>();
        try {
            List<Business> allSearch = BusinessDao.getAll2Page(t);
            for (Business all : allSearch) {
                Business search = new Business();
                search.setId(all.getId());
                allT.add(get(search));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return allT;
    }

    //统计指定条件总数
    public Integer count(Business t) {
        return BusinessDao.count(t);
    }

    //获取单条数据内容
    public Business get(Business t) {
        Business tSearch = null;
        try {
            tSearch = BusinessDao.get(t);


            //获取相关文件信息
            FileSystem fileSearchParam = new FileSystem();
            fileSearchParam.setRelevancy(String.valueOf(tSearch.getId()));
            fileSearchParam.setType(Business.modular);
            List<FileSystem> all = FileSystemService.getAll(fileSearchParam);
            if (all.size() > 0) {
                tSearch.setHasFile(true);
                tSearch.setFileList(all);
                tSearch.setFileListShow(all.get(0).getPath());
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
        return tSearch;
    }

    //实现数据添加内容
    public Business add(Business t) {
        try {
            Business add = BusinessDao.add(t);
            return add;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    }

    //实现数据批量添加内容
    public String addS(List<Business> t) {
        String isSuccess = "false";
        try {
            for (Business o : t) {
                final Business add = BusinessDao.add(o);
            }
            isSuccess = "true";
        } catch (Exception e) {
            e.printStackTrace();
            return "false-has-double";
        }
        return isSuccess;
    }

    //实现数据删除内容
    public String del(Business t) {
        String isSuccess = "false";
        try {
            isSuccess = BusinessDao.del(t);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return isSuccess;
    }

    //实现数据的更新操作
    public String update(Business t) {
        try {
            String isSuccess = BusinessDao.update(t);
            return isSuccess;
        } catch (Exception e) {
            e.printStackTrace();
            return "false-has-double";
        }
    }
}
